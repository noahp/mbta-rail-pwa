...
function startPolling() {
  if (pollTimer) return;
  pollTimer = window.setInterval(() => void pollAll(), (prefs.refreshInterval ?? 10) * 1000);
}
  ...
  <div class="settings-sheet">
    ...
    <div class="text-input-group">
      <input
        ...
        spellcheck="false"
      />
      <button class="input-icon-btn" data-action="toggle-api-key-visible"
      ...
      <button class="input-icon-btn" data-action="copy-api-key"
              aria-label="Copy API key" ${prefs.apiKey ? '' : 'disabled'}>
        ${
          apiKeyJustCopied
            ? `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
    ...
    <p class="field-hint">
      ...
      (takes ~1 day). Without a key you're limited to 20 req/min.
    ...
    <a class="build-version"
       href="https://github.com/noahp/mbta-rail-pwa/commits/${__BUILD_SHA__}"
       target="_blank" rel="noopener">
      ${__BUILD_SHA__.slice(0, 7)} · ${formatBuildTime(__BUILD_TIME__)}
    </a>
    <a class="github-link" href="https://github.com/noahp/mbta-rail-pwa" target="_blank" rel="noopener">
  ...
  document.body.addEventListener('click', (e: MouseEvent) => {
    ...
    switch (action) {
      ...
      case 'close-settings':
        ...
        if ((e.target as HTMLElement).closest('.settings-sheet')) break;
        showSettings = false;
        renderSettings();
        break;
      ...
      case 'toggle-api-key-visible':
        apiKeyVisible = !apiKeyVisible;
        renderSettings();
        break;

      case 'copy-api-key':
        if (!prefs.apiKey) break;
        void navigator.clipboard.writeText(prefs.apiKey).then(() => {
          apiKeyJustCopied = true;
          renderSettings();
          setTimeout(() => {
            apiKeyJustCopied = false;
      ...
      case 'set-interval': {
        const secs = parseInt(el.dataset.interval ?? '10', 10);
